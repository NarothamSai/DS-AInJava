// Circular Array not replacing if size is full
class Queue{
  ...
  public static void main(String[] args){
    ...
    for(int i = 0; i < 100; i++) testArr[i] = i;

    for(int i = 1; i < 12; i++) test.add(testArr[i]);
    for(int i = 0; i < 5; i++) System.out.println(test.remove());
    
    test.printElem();
    System.out.println("removed:"+test.remove());

    for(int i = 12; i < 20; i++) test.add(testArr[i]);
    test.printElem();

    for(int i = 0; i < 13; i++) System.out.println(test.remove());
    test.printElem();
